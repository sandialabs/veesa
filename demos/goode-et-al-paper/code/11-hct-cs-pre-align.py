...
# Cross-sectional approach (pre-alignment) to modeling H-CT data
...
for s in sparam:
  ...
  train_x = hct.load_object(data_results="data", train_test="train", stage="smoothed", sparam=s).T
  test_x = hct.load_object(data_results="data", train_test="test", stage="smoothed", sparam=s).T

  # Train neural network, compute performance metrics, and save results
  hct.apply_model(x=train_x, y=train_y, analysis_name="cs-pre-align", sparam=s, seed=20211213)

  # Load the model
  nn = hct.load_object(data_results="results", train_test="train", stage="nn-cs-pre-align", sparam=s)
  ...
  # Save PFI results
  hct.save_object(obj=pfi, data_results="results", train_test="train", stage="pfi-cs-pre-align", sparam=s)
  ...
  hct.save_object(
    ...
    train_test="test",
    stage="pred-and-metrics-cs-pre-align",
    sparam=s
  )
