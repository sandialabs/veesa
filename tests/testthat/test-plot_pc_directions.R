...
# Test 6b: Check that vfpca objects are handled. A vfPCA object has the same
#          f_pca structure as jfPCA but no 'eigs' element, so prop_var falls
#          back to the retained 'latent' spectrum.
...
# Test 11: Check that an explicit time vector and free y-axis scales are accepted
...
# Test 12: Check that hfpca honors an explicit times range (rescaled to the
#          number of warping-function samples)
test_that("hfpca rescales an explicit times range", {
  result <- plot_pc_directions(
    fpcs = 1:2,
    fdasrvf = mock_hfpca,
    fpca_method = "hfpca",
