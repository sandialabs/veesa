def save_object(obj, data_results, train_test, stage, sparam, sub=False):
  
  """ Apply smoothing to H-CT data  
    Args:
        obj: object to be saved
        data_results:  character string indicating whether the object is 'data' or 'results'
  ...
  # Save object
  if sub == False:
    fp = "../" + data_results + "/hct-" + train_test + "-" + stage + "-sparam" + str(sparam_name)  + ".pkl"
    pickle.dump(obj, open(fp, 'wb'))
  else: 
    fp = "../" + data_results + "/hct-sub-" + train_test + "-" + stage + "-sparam" + str(sparam_name)  + ".pkl"
    pickle.dump(obj, open(fp, 'wb'))
  
def load_object(data_results, train_test, stage, sparam):
...
def apply_model(x, y, analysis_name, sparam, seed):
  ...
  # Save neural network
  save_object(obj=nn, data_results="results", train_test="train", stage="nn-"+analysis_name, sparam=sparam)
  ...
  save_object(obj=nn_res, data_results="results", train_test="train", stage="pred-and-metrics-"+analysis_name, sparam=sparam)
