...
sim_train_nobs = dim(sim_train_wide)[1]
sim_test_nobs = dim(sim_test_wide)[1]

# Determine the time points where values are observed
sim_times = sort(unique(sim_train_long$t))
...
sim_data_cs_train = sim_train_wide %>% select(-id, -data) %>% mutate(group = factor(group))
sim_data_cs_test = sim_test_wide %>% select(-id, -data) %>% mutate(group = factor(group))

# cross-sectional approach
sim_rf_cs_fp = paste0(fp, "results/sim/sim-rf-cross-sectional.rds")
if (file.exists(sim_rf_cs_fp)) {
  sim_rf_cs = readRDS(sim_rf_cs_fp)
} else {
  set.seed(20210301)
  sim_rf_cs <-
  ...
  saveRDS(sim_rf_cs, sim_rf_cs_fp)
}

# Compute the model predictions on train/test data
sim_pred_cs_train = predict(sim_rf_cs, sim_data_cs_train)
sim_pred_cs_test = predict(sim_rf_cs, sim_data_cs_test)

# Compute the accuracy on train/test data
sim_acc_cs_train = sum(sim_pred_cs_train == sim_train_wide$group) / sim_train_nobs
sim_acc_cs_test = sum(sim_pred_cs_test == sim_test_wide$group) / sim_test_nobs

# Compute PFI using accuracy for the cross-sectional approach
sim_pfi_acc_cs_fp = paste0(fp, "results/sim/sim-pfi-acc-cross-sectional.rds")
if (file.exists(sim_pfi_acc_cs_fp)) {
  sim_pfi_acc_cs = readRDS(sim_pfi_acc_cs_fp)
} else {
  set.seed(20210921)
  sim_pfi_acc_cs <- 
    compute_pfi(
      x = sim_data_cs_test[, -1],
      y = sim_data_cs_test$group,
      f = sim_rf_cs,
      K = sim_K, 
      metric = "accuracy"
    )
  saveRDS(sim_pfi_acc_cs, sim_pfi_acc_cs_fp)
}

# Compute PFI using log-loss for the cross-sectional approach
sim_pfi_ll_cs_fp = paste0(fp, "results/sim/sim-pfi-ll-cross-sectional.rds")
if (file.exists(sim_pfi_ll_cs_fp)) {
  sim_pfi_ll_cs = readRDS(sim_pfi_ll_cs_fp)
} else {
  set.seed(20210921)
  sim_pfi_ll_cs <- 
    compute_pfi(
  ...
  saveRDS(sim_pfi_ll_cs, sim_pfi_ll_cs_fp)
}
...
sim_train_acc_jfpca = sum(sim_train_pred_jfpca == sim_train_model_data$group) / sim_train_nobs
sim_test_acc_jfpca = sum(sim_test_pred_jfpca == sim_test_model_data$group) / sim_test_nobs

# Compute PFI using log-loss with the VEESA pipeline
sim_pfi_ll_jfpca_fp = paste0(fp, "results/sim/sim-pfi-ll-jfpca.rds")
if (file.exists(sim_pfi_ll_jfpca_fp)) {
  sim_pfi_ll_jfpca = readRDS(sim_pfi_ll_jfpca_fp)
} else {
  set.seed(20210921)
  sim_pfi_ll_jfpca <-
    compute_pfi(
  ...
  saveRDS(sim_pfi_ll_jfpca, sim_pfi_ll_jfpca_fp)
}
...
## ----sim-check--------------
if (data.frame(sim_pfi_acc_cs$pfi_single_reps) %>% summarise_all(.funs = sd) %>% as.numeric() %>% unique() > 0)
  stop("Some times have PFI standard deviation > 0.")


## ----sim-pfi-cutoff---------
sim_veesa_pfi_cutoff = 0.02
...
plot_sim_cs_means <-
  ...
  theme(legend.position = "bottom", aspect.ratio = sim_ar) +
  ylim(-0.15, 1.4)

# Plot the PFI from the cross-sectional approach (using accuracy as the metric)
plot_cs_pfi_acc <-
  data.frame(
    time = sort(unique(sim_data$t)),
    imp = sim_pfi_acc_cs$pfi
  ) %>%
...
plot_veesa_pfi_ll <-
  data.frame(fpc = 1:length(sim_pfi_ll_jfpca$pfi),
             imp = sim_pfi_ll_jfpca$pfi) %>%
...
## ----figS3, fig.width = 16, fig.height = 12, out.width = "6.5in", fig.cap = paste0("Principal direction plots of the VEESA pipeline jfPCs with log-loss based PFI values greater than ", sim_veesa_pfi_cutoff, " (excluding jfPCs 1 and 2 shown in Figures 2 and 4).")----

# Specify the figure size (for determining font size)
fS3_fs = 18.5

sim_veesa_pc_top <- 
  data.frame(pfi = sim_pfi_ll_jfpca$pfi) %>%
  mutate(pc = 1:n()) %>%
  filter(pfi > sim_veesa_pfi_cutoff, pc != 1, pc !=2) %>%
  arrange(desc(pfi)) %>%
  pull(pc)

plot_pc_directions(
  fpc = sim_veesa_pc_top,
